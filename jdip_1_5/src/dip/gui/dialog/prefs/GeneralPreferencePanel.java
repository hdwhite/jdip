//
//  @(#)GeneralPreferencePanel.java	1.00	4/1/2002
//
...
public class GeneralPreferencePanel extends PreferencePanel
{
	...
	public GeneralPreferencePanel(final ClientFrame cf)
	{
		...
		// create clear panel
		JPanel clrPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		...
		// layout 
		int h1[] = { BORDER, 0,5, 0,5, 0,5, 0,5, 0,5, 0,0, 5,0, 0, 0, BORDER };
		int w1[] = { BORDER, 0, 10, 0, 5, 0, 5, BORDER };
		...
		add(new JPanel(), c.rcwh(6,2,4,1,"l"));		// this is empty, now (can add a checkbox item here)
		add(new JPanel(), c.rcwh(8,2,4,1,"l"));		// this is empty, now (can add a checkbox item here)
		
		add(new JLabel( Utils.getLocalString(GPP_SAVE_DIR_TEXT) ), c.rcwh(12,2,2,1,"l"));
		add(saveDir, c.rcwh(13,2,1,1,"lr"));
		add(browseSaveDir, c.rcwh(13,4,1,1,"lrtb"));
		
		add(clrPanel, c.rcwh(15,2,2,1,"ltb"));
	...
	private void directoryBrowse()
	{
		...
		File dirPath = chooser.displaySelectDir(cf, Utils.getLocalString(DIALOG_TITLE));
		chooser.dispose();
