...
public class ExportPreferencePanel extends PreferencePanel
{
	...
	*/
	private void makeAndSetGUIItems()
	{
		...
		// png BPP combobox
		String[] choices = new String[ALLOWED_PNG_BPP.length];
		choices[0] = Utils.getLocalString(I18N_CHOICE_PNG_BPP_UNLIM);
		for(int i=1; i<choices.length; i++)
		{
			choices[i] = String.valueOf( (int) Math.pow(2, ALLOWED_PNG_BPP[i]) );
		}
		pngBPP = new JComboBox(choices);
		pngBPP.setEditable(false);
		pngBPP.setSelectedIndex( bppToCBIdx(bpp) );
	...
	public void apply()
	{
		...
		prefs.putInt(NODE_PNG_BPP, cbIdxtoBPP(pngBPP.getSelectedIndex()) );
	...
	*/
	...
	/** Returns True if BPP for indexed PNG is a valid value */
	private static boolean checkBPP(int bpp)
	{
		for(int i=0; i<ALLOWED_PNG_BPP.length; i++)
		{
			if(bpp == ALLOWED_PNG_BPP[i])
			{
				return true;
			}
		}
	...
	/** Get the BitsPerPixel from the combobox index */
	private int cbIdxtoBPP(int cbIdx)
	{
		return ALLOWED_PNG_BPP[cbIdx];
	}// cbIdxtoBPP()
	
	/** Get the combobox Index from the BitsPerPixel */
	private int bppToCBIdx(int bpp)
	{
		for(int i=0; i<ALLOWED_PNG_BPP.length; i++)
		{
			if(bpp == ALLOWED_PNG_BPP[i])
		...
		throw new IllegalArgumentException(String.valueOf(bpp));
	}// bppToCBIdx()
